...
def excelGeneration(imgName, dataFrames):

    with pd.ExcelWriter(imgName) as writer:
...
def extractMain(ocrEngine, tablebboxDtls, cvImg, outPath, fileName):
    ...
    excelGeneration(outPath + "/" + fileName + '.xlsx', allDataframes)
    fSting = f"EXCEL GENETAION COMPLETED FOR {fileName.split('_page')[0]} IN {outPath.replace('./','')} FOLDER\n"
